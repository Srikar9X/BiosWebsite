...
var connection = mysql.createConnection({host:'localhost', user:'root', password:'', database:'Biosdata'});
          ...
          connection.query('SELECT * FROM users WHERE Username = ?', [usernameuse], function (error, results, fields) {
               if (results.length == 1)
               {
                    ...
                    var transporter = mailsender.createTransport({
                         service: 'gmail',
                         auth: {
                              user: 'xzeltronx@gmail.com',
                              pass: '((#Zeltron%'
                         }
                    });

                    var mailOptions = {
                         from: 'zeltronsrikar@gmail.com',
                         to: emailuse,
...
app.post('/forums', function(req, res) {
     res.redirect('/forumspage');
});

app.get('/forumspage', function(req, res) {
